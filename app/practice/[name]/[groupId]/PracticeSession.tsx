...
// For typing phase: lowercase + trim only — accents ARE required
...
export function PracticeSession({
  ...
  const buildDone = (p: { phase: string; wordId: string }[]) =>
    new Set(p.map((x) => `${x.phase}:${x.wordId}`));
  ...
  // Reset when phase or word changes
  useEffect(() => {
    ...
    if (phase === "match_es_en" || phase === "match_en_es") {
      ...
      setOptions(buildOptions(words, word, field));
    }
    ...
    // words is stable; only word/phase should re-trigger
  ...
  async function advance() {
    ...
    // Fire-and-forget save — UI advances immediately
    fetch("/api/progress", {
      method: "POST",
  ...
  function handleScrambleChar(ch: string) {
    ...
    // Case-insensitive comparison; display the canonical char
    if (ch.toLowerCase() === expected.toLowerCase() || ch === expected) {
      const newChars = scramChars.map((c, i) =>
        i === scramPos ? { ch: expected, ok: true } : c
      );
      setScramChars(newChars);
      const next = scramPos + 1;
      setScramPos(next);
      if (next >= word.spanish.length) {
    ...
    return (
      <div className="max-w-3xl mx-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <Link href={`/student/${encodedName}`} className="text-blue-600 hover:underline text-lg">
            ← Grupos
          </Link>
          ...
          {words.map((w) => (
            ...
            >
              ...
              <div className={`${isWords ? "p-3" : "p-4"}`}>
                <p className={`font-bold text-gray-800 text-center leading-tight ${isWords ? "text-base" : "text-lg"}`}>
                  {w.spanish}
                </p>
                <p className={`text-gray-400 text-center mt-1 leading-tight ${isWords ? "text-sm" : "text-base"}`}>
                  {w.english}
                </p>
              </div>
            </div>
  ...
  if (phase === "complete") {
    return (
      <div className="max-w-lg mx-auto text-center py-20 px-8">
        ...
        <Link
          href={`/student/${encodedName}`}
          className="inline-block text-xl font-bold px-10 py-5 bg-green-500 text-white rounded-2xl hover:bg-green-600 transition-all shadow-md"
        >
          ← Volver a mis grupos
        </Link>
        <div className="mt-4">
          <button
            ...
            className="text-gray-400 hover:text-gray-600 text-base underline"
          >
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="w-full bg-gray-200 rounded-full h-5 mb-8 overflow-hidden">
        <div
          className="h-5 rounded-full bg-blue-500 transition-all duration-500"
          style={{ width: `${mastery}%` }}
        />
      </div>
      ...
      {(phase === "match_es_en" || phase === "match_en_es") && (
        <div>
          <div className="flex justify-center mb-5">
            <img
              src={word.imageUrl ?? FALLBACK_IMG}
              alt=""
              className="h-44 w-44 object-cover rounded-2xl shadow-md"
            />
          </div>
                ...
                <button
                  ...
                  className={`text-xl font-bold py-6 px-4 rounded-2xl border-4 transition-all ${
                    isCorrect
                      ...
                      : isWrong
                      ? "border-red-500 bg-red-50 text-red-500 opacity-60 cursor-not-allowed"
      ...
      {phase === "unscramble" && (
        <div>
          ...
          <div className="text-center text-3xl font-semibold text-gray-500 mb-7">
            {word.english}
          </div>

          {/* Clickable area that captures keyboard */}
          <div
            ref={scramRef}
            tabIndex={0}
          ...
          >
            {scramChars.map((c, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`w-11 h-13 min-w-[2.75rem] h-[3.25rem] flex items-center justify-center text-2xl font-bold rounded-xl border-[3px] transition-all ${
                    isCurrent
                      ? scramFlash
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-3">
            {[
              "á", "é", "í", "ó", "ú", "ñ",
              ...(groupType === "phrases" ? ["Á", "É", "Í", "Ó", "Ú", "Ñ"] : []),
            ].map((ch) => (
              <button
                key={ch}
                onMouseDown={(e) => {
                  e.preventDefault(); // keep focus on scramRef
                  handleScrambleChar(ch);
                }}
          ...
          <p className="text-center text-gray-400 text-sm">
            Haz clic aquí arriba y escribe con el teclado
          </p>
        </div>
      )}
      ...
      {(phase === "type_en" || phase === "type_es") && (
        <div>
          ...
          <form onSubmit={handleTypingSubmit} className="flex gap-3 max-w-md mx-auto">
            ...
            <button
              type="submit"
              className="text-2xl font-bold px-6 py-4 bg-green-500 text-white rounded-2xl hover:bg-green-600 active:scale-95 transition-all shadow-sm"
            >
              ✓
            </button>
          </form>

          {/* Special chars for typing phase */}
          ...
          <div className="text-center mt-5 h-8">
            ...
            {typCorrect && (
              <p className="text-green-500 font-bold text-xl">✓ ¡Correcto!</p>
            )}
          </div>
        </div>
      )}
